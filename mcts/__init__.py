...
def mcts_do_chess(model, simulate_count, cur_state, cur_player):
    conf = MonteChessTreeConfig()
    tree = MonteChessTree(conf, model)
    s = MonteChessState(conf)
    s.current_player = cur_player
    s.chess_state = np.array(cur_state, dtype=np.int)
    tree.reset(s)
    cur_node = tree.root
    backward_count = 0
    while backward_count < simulate_count:
